...
2. **Раскрывающиеся секции под каждым компонентом** в `components.html` —
   автоматический парсинг `styles/components/<name>.css` для перечисления
   используемых `var(--token)` и зависимостей-импортов. Под каждой
   demo-карточкой показывает: «токены: …», «зависимости: …». Делаем через
   общий разворачиваемый блок. Парсер собирает данные при build (или JS
   в браузере читает CSS).
